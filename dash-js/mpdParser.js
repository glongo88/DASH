...
function MPDParser(__mpd)
{
	...
	this.pmpd = new Object();		//oggetto che vado creando (analizzando mpd aggiungo nodi e attributi qui)
	this.rmpd = new Object();		
	__mpd.trim();
	__mpd.replace(/(\r\n)/g, "");
	this.mpd = this.parser(__mpd);	//mpd = oggetto che rappresenta il file parsificato (return del parser)
}
...
{
	console.log('Found a new segmentList...');
		...
		if(segmentListChilds.item(s).nodeName == MPD.segmentURL.name)
		{
			...
			for(i=1; i<attribs; i++)
			{
				...
				if(node.hasAttribute(attribValue)) {
					eval("this.pmpd.period[periods].group[groups].representation[representations].segmentList.segment[segments]." + getKeyByIndex(MPD.segmentURL,i).toString() + "= node.attributes.getNamedItem(attribValue).value");
					...
					//console.log('Found a new segment: '+node.attributes.getNamedItem(attribValue).value);
				}
			}
			segments++;		
		}	
...
{
	console.log('Found a new representation...');
...
MPDParser.prototype.parseSegmentTemplate = function(representations, periods, groups, node)
{
	...
	for(i=1; i<attribs; i++)
	{
		...
		if(node.hasAttribute(attribValue)) eval("this.pmpd.period[periods].group[groups].segmentTemplate." + getKeyByIndex(MPD.segmentTemplate,i).toString() + "= node.attributes.getNamedItem(attribValue).value");
	...
	this.parseAvailabilityStartTime();

}

MPDParser.prototype.parseGroup = function(periods, groups, node)
{
	console.log('Found a new adaptationSet...');
...
{
	console.log('Found a new period...');
	var attribs =objectSize(MPD.period);
	for(i=1; i<attribs; i++)
	{
		//scorro tutti gli attributi che trovo in mpd e li aggiungo a pmdp
		...
		for(c=0;c<childsFromRoot.length;c++)
		{
			if(childsFromRoot.item(c).nodeName != "#text")
			{
				...
				if(node.nodeName == MPD.baseURL.name)
				{
					...
					console.log('BaseURL='+this.pmpd.baseURL);
				}
...
MPDParser.prototype.parseAvailabilityStartTime = function() {
	console.log("Entering parseAvailabilityStartTime()...");
	console.log("MPD availabilityStartTime: " + this.pmpd.availabilityStartTime);
	
	var parseDateI = this.pmpd.availabilityStartTime.split("T");
	...
	if (d1==d2) {
		...
		var diffTot = diffS + diffM + diffH;
		var currSegNum = Math.ceil(diffTot / this.pmpd.period[0].group[0].segmentTemplate.duration);
		
		console.log("Live started " + diffTot + " seconds ago...");
		console.log("Current segment should be number:" + currSegNum);
