...
{
	console.log('Entering init_mediaSourceBuffer()...');
	...
	mediaSourceBuffer.prototype.bufferStateListener = function(object){
		...
		console.log('Entering bufferStateListener...');
		
		if ( countSegmentRequest == 100 ) {
			console.log("Error: too many requests. Segment not available.");
			return;
		}
		
		if ( segNotAvailable == 1 ) {
			console.log('segNotAvailable == 1');
			extCount--;
			segNotAvailable = 0;
			countSegmentRequest++;
			window.setTimeout(function () {_dashFetchSegmentAsynchron(object);}, 1000);
			window.setTimeout(function () {_mediaSourceBuffer.bufferStateListener(_mediaSourceBuffer);}, 1000);
			return;
		}
		
		//console.log('object.mediaElementBuffered: '+object.mediaElementBuffered+' - dashPlayer.videoTag.currentTime: '+dashPlayer.videoTag.currentTime+' - object.lastTime: '+object.lastTime);
    ...
    // this is the callback method, called by the AJAX xmlhttp call
   	mediaSourceBuffer.prototype.callback = function(){
			...
			console.log('Callback method AJAX call...');
	...
	mediaSourceBuffer.prototype.refill = function(object){
        ...
        if(object.doRefill == true){
            ...
            if(object.fillState.seconds < object.bufferSize.maxseconds){
				...
				console.log('refill() setting doRefill to false...');
